...
file_path = "./object/data/info.json"
...
class similarity():
    ...
    def get_similar_hair_list(self,target_path, block_size = 20):

        total = []

        target_mask = self.get_mask(target_path)
        sum = 0
        for model in self.models:
            file_name = './object/data/ssim/' + model['ssim'] +'.png'
            ...
            for i in range(num):
                ...
                # 너비 길이 맞추고 조합
                max_w = max(t_w, l_w)
                target_mask = cv2.resize(target_mask, dsize=(0, 0), fx=(max_w / t_w), fy=(max_w / t_w))
...
list = similarity().get_similar_hair_list('./notitle4.png',block_size= 20)
print(list[:6])

for i in list:
    front_R, front_L, base = similarity().get_part(i['name'])
    print(front_R, front_L, base)