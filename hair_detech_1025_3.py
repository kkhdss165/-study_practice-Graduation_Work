...
class post_processing:
    ...
    def contour_process(self, img, center, join_ratio = 1.0):
        ...
        # 마스킹
        print(back_img2.shape, self.img.shape)
        back_img2 = cv2.bitwise_and(back_img2,self.img)
...
class compare_hair:
    def __init__(self, img):
        self.dir_path = './ssim'
        self.size = img.shape
        self.choose_image = img
        file_name = self.compare()
        ...
        for (root, directories, files) in os.walk(self.dir_path):
            for file in files:
                if '.png' in file:
                    ...
                    length = min(h, w, self.size[0])
                    print("self.size = ", self.size)
                    compare_img = cv2.resize(compare_img, (length, length))
                    choose_img = cv2.resize(self.choose_image, (length, length))


                    mses = mse(compare_img, choose_img)
                    rmses = rmse(compare_img, choose_img)
                    psnrs = psnr(compare_img, choose_img)
                    ssims = ssim(compare_img, choose_img)
                    uqis = uqi(compare_img, choose_img)
                    msssims = msssim(compare_img, choose_img)
                    ergass = ergas(compare_img, choose_img)
                    sccs = scc(compare_img, choose_img)
                    rases = rase(compare_img, choose_img)
                    sams = sam(compare_img, choose_img)
                    vifps = vifp(compare_img, choose_img)
                    ...
                    mselist.append(mses)
                    rmselist.append(rmses)
                    psnrlist.append(psnrs)
                    ssimlist.append(ssims)
                    uqilist.append(uqis)
                    msssimlist.append(msssims)
                    ergaslist.append(ergass)
                    scclist.append(sccs)
                    raselist.append(rases)
                    samlist.append(sams)
                    vifplist.append(vifps)
