...
def groupingPixelGroup(image, group_pixel_list, center, size = 5):
    ...
    for now_index in range(len(combine_list)):
        ...
        near_group_color = []
        # 주변 그룹 서치
        if now_color not in [center[0]]:
            for index in combine_list[now_index]:
                for search_index in range(len(combine_list)):
                    if search_index != now_index:
                        for around in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                            if (index[0] + around[0], index[1] + around[1]) in combine_list[search_index]:
                                if search_index not in near_group_list:
                                    n_index_y = combine_list[search_index][0][0]
                                    n_index_x = combine_list[search_index][0][1]
                                    near_color = combine_image[n_index_y  * size,n_index_x * size].tolist()
                                    if near_color not in [center[0]]:
                                        near_group_list.append(search_index)
                                        near_group_color.append(near_color)
        ...
        if len(near_group_list) != 0:
            ...
            for idx in combine_list[now_index]:
                if max_x == None:
                    max_x = idx[1]
                    min_x = idx[1]
                    max_y = idx[0]
            ...
            for x_index in range(min_x, max_x + 1):
                for y_index in range(min_y, max_y + 1):
                    if (y_index,x_index) not in combine_list[now_index]:
            ...
            if n_total > 0:
                ...
                for near_color in near_group_color:
                    diff = (near_color[0]-avg_color[0])**2 + (near_color[1]-avg_color[1])**2 + (near_color[2]-avg_color[2])**2
                ...
                print("combine_index",combine_index)
                ...
                for near_index in near_group_list:
                    for n_index in combine_list[near_index]:
                        temp2[n_index[0] * size:(n_index[0] + 1) * size, n_index[1] * size:(n_index[1] + 1) * size] = combine_image[n_index[0] * size,n_index[1] * size].tolist()
                ...
                #변경 업데이트
                # 변경후 업데이트
                # print(after_combine_list[now_index ])
...
if __name__ == "__main__":
    for file_name in file_names:
        ...
        K_image, center = kmeasImage(image, 20)
