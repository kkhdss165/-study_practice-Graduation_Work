...
OUTLINE_POINTS_1 = [10, 109, 67, 103, 54, 21, 162, 127, 234, 93, 132, 58, 172, 136, 150, 149, 176, 148, 152, 377, 400,
                    378, 379, 365, 397, 288, 361, 323, 454, 356, 389, 251, 284, 332, 297, 338]  # 36개
OUTLINE_POINTS_2 = [151, 108, 69, 104, 68, 71, 139, 34, 227, 137, 177, 215, 138, 135, 169, 170, 140, 171, 175, 396, 369,
                    395, 394, 364, 367, 435, 401, 366, 447, 264, 368, 301, 298, 333, 299, 337]  # 36개
OUTLINE_POINTS_3 = [9, 107, 66, 105, 63, 70, 156, 143, 116, 123, 147, 213, 192, 214, 210, 211, 32, 208, 199, 428, 262,
                    431, 430, 434, 416, 433, 376, 352, 345, 372, 383, 300, 293, 334, 296, 336]  # 36개
OUTLINE_POINTS_4 = [8, 55, 65, 52, 53, 46, 124, 35, 111, 117, 50, 187, 207, 216, 212, 202, 204, 194, 201, 200, 421, 418,
                    424, 422, 432, 436, 427, 411, 280, 346, 340, 265, 353, 276, 283, 282, 295, 285]  # 38개
...
file_names = ['../img/longhair.jpg', '../img/actor.jpg', '../img/hyunbin.png', '../img/jungmin.png',
              '../img/sangsoo.png', '../img/sukhun.png', '../img/longhair.jpg']
    ...
    for i in range(len(name)):
        if name[i] not in [center[0]]:
            if min_diff == None:
                ...
                min_diff = (avg_color[0] - name[i][0]) ** 2 + (avg_color[1] - name[i][1]) ** 2 + (
                            avg_color[2] - name[i][2]) ** 2

            else:
                now_diff = (avg_color[0] - name[i][0]) ** 2 + (avg_color[1] - name[i][1]) ** 2 + (
                            avg_color[2] - name[i][2]) ** 2
    ...
    for i in range(0, int(img_y / size)):
        for j in range(0, int(img_x / size)):
            ...
            for idx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                if ((i + idx[0]) * size >= 0 and (i + idx[0]) * size < img_y and (j + idx[1]) * size >= 0 and (
                        j + idx[1]) * size < img_x):
    ...
    while len(combine_list) > 2:
        ...
        if len(near_group_list) > 0:
            ...
            for near_color in near_group_color:
                ...
                diff = (near_color[0] - avg_color[0]) ** 2 + (near_color[1] - avg_color[1]) ** 2 + (
                            near_color[2] - avg_color[2]) ** 2 + 1
                # diff_hsv = (near_hsv[0] - avg_hsv[0]) ** 2 + (near_hsv[1] - avg_hsv[1]) ** 2 + (near_hsv[2] - avg_hsv[2]) ** 2 + 1
                diff_hsv = (near_hsv[0] - avg_hsv[0]) ** 2 + (near_hsv[2] - avg_hsv[2]) ** 2 + 1
            ...
            for n in range(len(near_group_color)):
                sum = diff_list[n]/max(diff_list) + diff_hsv_list[n]/max(diff_hsv_list)
                diff_sum.append(sum)

            min_index = diff_list.index(min(diff_list))
            min_index = diff_hsv_list.index(min(diff_hsv_list))
            min_index = diff_sum.index(min (diff_sum))
            ...
            for index in near_pixel_list:
                temp2[index[0] * size:(index[0] + 1) * size, index[1] * size:(index[1] + 1) * size] = combine_image[
                    index[0] * size, index[1] * size].tolist()
            ...
            if now_index != combine_index:
                for points in combine_list[now_index]:
                    ...
                    combine_image[points[0] * size:(points[0] + 1) * size, points[1] * size:(points[1] + 1) * size] = \
                    near_group_color[min_index]
