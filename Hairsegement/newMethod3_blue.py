...
##
face_img_ycrcb = cv2.cvtColor(img_bit, cv2.COLOR_BGR2YCrCb)
lower = np.array([0,133,77], dtype = np.uint8)
upper = np.array([255,173,127], dtype = np.uint8)

skin_msk = cv2.inRange(face_img_ycrcb, lower, upper)
print(skin_msk)
skin = cv2.bitwise_and(img_bit, img_bit, mask = skin_msk)
img_temp = skin.copy()
img_temp[:,:,0], img_temp[:,:,1], img_temp[:,:,2] = np.average(skin, axis=(0,1))

cv2.imshow("img_temp", skin)
cv2.waitKey(0)

blue_mask = np.full_like(img_bit,(255,0 ,255))
blue_wise = cv2.bitwise_and(img_bit, blue_mask)

K = 8    # 군집화 갯수(16컬러) ---①
data = blue_wise.reshape((-1,3)).astype(np.float32)
# 반복 중지 요건 ---③
...
def getmostColor():
    for facial_landmarks in result.multi_face_landmarks:
        ...
        bit_and = cv2.bitwise_and(mask, temp)
...
for facial_landmarks in result.multi_face_landmarks:
    ...
    for i in RIGHT_EYE_POINT_2:
        ...
        list.append([int(pt1.x * img_x), int(pt1.y * img_y)])
    list = np.array(list,np.int32)
    cv2.fillPoly(temp, [list], mostcolor[0])
...
center_hsv = cv2.cvtColor(pixel, cv2.COLOR_BGR2YCR_CB)
center_hsv = center[0]
...
for i in range(len(center)):
    ...
    for j in range(i+1, len(center)):
        ...
        if center_hsv[max_index][0] < center_hsv[j][0]:
            max_index = j
...
before = temp[start_point[1]][start_point[0]]
diff = []
...
for i in range(start_point[1],0,-1):
    ...
    if abs(now_index - before_index) > 1 :
        cv2.line(temp_copy , [start_point[0], i], [start_point[0], i], (255, 0, 255), 2)
        diff.append(i)
    else:
...
# cv2.line(temp, [start_point[0] , start_point[1]-max_index], [start_point[0] , start_point[1]-max_index], (0, 0, 255), 5)
