...
def groupingPixel(image, center, size = 5):
    ...
    for i in range(len(pixel_nums)):
        ...
        for idx in range(len(g_near_index)):
            diff = (g_near_colors[idx][0] - now_color[0])**2 + (g_near_colors[idx][1] - now_color[1])**2 + (g_near_colors[idx][2] - now_color[2])**2
            g_diff.append(diff)
        ...
        if now_group_index != combine_index:
            for points in after_combine_list[now_group_index]:
                ...
                after_combine_image[points[0] * size:(points[0] + 1) * size, points[1] * size:(points[1] + 1) * size] = g_near_colors[min_index]
        ...
        print(depart_group[now_group_index])
