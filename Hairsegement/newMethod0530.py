...
def groupingPixel(image, size = 1):
    Total_group =[]
    depart_group =[]
    colors=[]
    img_y, img_x, _ = image.shape
    for i in range(0,int(img_y/size)):
        for j in range(0,int(img_x/size)):
            ...
            for x in [-1,0,1]:
                for y in [-1,0,1]:
                    if (x != 0 or y != 0) and ((i+y)*size >= 0 and (i+y)*size < img_y and (j+x)*size >=0 and (j+x)*size < img_x):
                        arround.append(image[(i+y)*size][(j+x)*size].tolist())
            ...
            Total_group[index].append((i*size,j*size))

    print(colors)
    for i in range(len(colors)):
        temp = image.copy()
        for index in Total_group[i]:
            temp[index[0]:index[0]+size,index[1]:index[1]+size] = [0,255,0]
...
def mosaic(image):
    img_y, img_x, _ = image.shape
    size = 5
    for i in range(int(img_y/5)):
        for j in range(int(img_x/5)):
            arround =[]
            for x in range(5):
                for y in range(5):
                    arround.append(image[5*i+x][5*j+y].tolist())

            for x in range(5):
                for y in range(5):
                    image[5*i+x][5*j+y] = getFrequent(arround)
...
if __name__ == "__main__":
    for file_name in file_names:
        ...
        K_image, center = kmeasImage(image, 10)

        center = showCenter(center)
        K_image, start_point = removeEyesandMouse(K_image)

        image = mosaic(K_image)

        K_image = groupingPixel(K_image, 5)
