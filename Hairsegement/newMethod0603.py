...
def groupingPixel(image, center, size = 5):
    ...
    for i in range(len(pixel_nums)):
        ...
        if len(near_index) != 0:
            ...
            # 현재 그룹에 직접 접해있는 픽셀들의 평균색을 구하기
            sum_b = now_color[0] * len(depart_group[now_group_index])
            sum_g = now_color[1] * len(depart_group[now_group_index])
            sum_r = now_color[2] * len(depart_group[now_group_index])
            num_total = len(depart_group[now_group_index])
            for idx in range(len(near_index)):
                n_pixel = adjoin_pixel_nums[idx]
                n_pixel = 1
                sum_b += near_colors[idx][0] * n_pixel
                sum_g += near_colors[idx][1] * n_pixel
                sum_r += near_colors[idx][2] * n_pixel
                num_total += n_pixel


            avg_color = [sum_b/num_total, sum_g/num_total, sum_r/num_total]

            #현재 픽셀, 평균 픽셀, 주변 픽셀의 시각화
            color_bar = np.zeros((150,150*(2+len(near_index)),3),np.uint8)
            color_bar[:,:150] = now_color
            color_bar[:,150:300] = (int(avg_color[0]),int(avg_color[1]),int(avg_color[2]))
            for idx in range(len(near_index)):
                color_bar[:,150*(idx+2):150*(idx+3)] = near_colors[idx]
            ...
            for idx in range(len(near_index)):
                # print(now_color, near_colors[idx])
                # color_diff = abs((now_color[0]+now_color[1]+now_color[1])+(near_colors[idx][0]+near_colors[idx][1]+near_colors[idx][2]))
                # color_diff = abs(now_color[0]-near_colors[idx][0]) + abs(now_color[1]-near_colors[idx][1]) + abs(now_color[2]-near_colors[idx][2])

                sum = (avg_color[0]-near_colors[idx][0])**2 + (avg_color[1]-near_colors[idx][1])**2 + (avg_color[2]-near_colors[idx][2])**2
                print(idx,sum, lens[idx])
                sum = sum
            ...
            if now_group_index != combine_index:
                ...
                for idx in near_list[now_group_index]:
                    ...
                    #     print(near_list[idx])
                        ...
                        # print(near_list[idx])
                        if combine_index not in near_list[idx] and combine_index != idx:
