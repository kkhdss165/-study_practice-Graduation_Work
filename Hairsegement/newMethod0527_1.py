...
def groupingPixel(image):
    Total_group =[]
    colors=[]
    img_y, img_x, _ = image.shape
    check = np.zeros((img_x,img_y))
    for i in range(1,img_y-1):
        for j in range(1,img_x-1):

            arround = []
            for x in [-1,1]:
                for y in [-1,1]:
                    if x != 0 or y != 0:
                        arround.append(image[i+x][j+y].tolist())

            if image[i][j].tolist() not in arround:
                image[i][j] = getFrequent(arround)

            if image[i][j].tolist() not in colors:
                colors.append(image[i][j].tolist())
                Total_group.append([])

            index = colors.index(image[i][j].tolist())
