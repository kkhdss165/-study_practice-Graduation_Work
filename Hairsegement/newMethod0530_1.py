...
def groupingPixel(image, center, size = 1):
    Total_group =[]
    depart_group =[]
    colors=[]
    img_y, img_x, _ = image.shape
    ...
    for i in range(len(pixel_nums)):
        ...
        for now_near_index in near_list[now_group_index]:
            ...
            print("now:" ,now_group_index, now_color,len(depart_group[now_group_index]),
                  "near:",now_near_index,image[near_y][near_x],len(after_combine_list[now_near_index]))

            # 조건
            if image[near_y][near_x].tolist() not in [center[0]] and image[now_y][now_x].tolist() not in [center[0]]:
                if len(after_combine_list[now_near_index]) >= len(depart_group[now_group_index]):
                    ...
                    near_colors.append(image[near_y][near_x].tolist())
        ...
        if len(near_index) != 0:
            ...
            for idx in range(len(near_index)):
                ...
                sum = color_diff + lens[idx]*lens[idx]
                # print(color_diff, lens[idx]*lens[idx] )
                print(now_color, near_colors[idx], color_diff)
                compare_list.append(sum)
            print(compare_list)
            combine_index = near_index[compare_list.index(min(compare_list))]
...
if __name__ == "__main__":
    for file_name in file_names:
        ...
        #
        # getHairArea(image, hair_list, start_point)
