...
def groupingPixel(image, center, size = 5):
        ...
        for j in range(0,int(img_x/size)):
            ...
            for idx in [(-1,0),(1,0),(0,-1),(0,1)]:
                    if ((i+idx[0])*size >= 0 and (i+idx[0])*size < img_y and (j+idx[1])*size >=0 and (j+idx[1])*size < img_x):
                        if image[(i+idx[0])*size][(j+idx[1])*size].tolist() not in [center[0]] and image[i*size][j*size].tolist() not in [center[0]] :
                            arround.append(image[(i+idx[0])*size][(j+idx[1])*size].tolist())

            if image[i*size][j*size].tolist() not in arround and len(arround) > 0:
    ...
    for i in range(len(pixel_nums)):
        ...
        now_color = after_combine_image[now_y][now_x].tolist()
        n_now_pixel = len(depart_group[now_group_index])
        ...
        for now_near_index in near_list[now_group_index]:
            ...
            near_color = after_combine_image[near_y][near_x].tolist()
            n_near_pixel = len(depart_group[now_near_index])
            ...
            # 조건
            print("now:", now_group_index, now_color, n_now_pixel, "near:", now_near_index, near_color, n_near_pixel)
        ...
        if len(g_near_index) != 0:
            ...
            for idx in depart_group[now_group_index]:
                if max_x == None:
                    max_x = idx[1]
                    min_x = idx[1]
                    max_y = idx[0]
            ...
            for point in after_combine_list[now_group_index]:
                process_image[point[0] * size:(point[0] + 1) * size, point[1] * size:(point[1] + 1) * size] = [0, 255,
                                                                                                               0]

            copy_image = after_combine_image.copy()
            ...
            if now_group_index != combine_index:
                for points in after_combine_list[now_group_index]:
                    ...
                    after_combine_image[points[0] * size:(points[0] + 1) * size,
