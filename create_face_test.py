...
class createFaceMesh:
    ...
    def import_head(self):
        ...
        bpy.ops.import_scene.obj(filepath="./static/headpart5.obj")
    ...
    def resize_rescale(self, object_name):
        recommand_scale_y = 1.076291202645727
        recommand_scale_z = 1.030298620748735
        self.chooes_object(object_name)
        obj = bpy.context.active_object
        obj.scale[1] *= 1.2
        list = [(obj.matrix_world @ v.co) for v in obj.data.vertices]

        h_max_x = max(ls[0] for ls in list)
        h_max_y = max(ls[1] for ls in list)
        h_max_z = max(ls[2] for ls in list)

        h_min_x = min(ls[0] for ls in list)
        h_min_y = min(ls[1] for ls in list)
        h_min_z = min(ls[2] for ls in list)

        h_length_x = h_max_x - h_min_x
        h_length_y = h_max_y - h_min_y
        h_length_z = h_max_z - h_min_z

        print("길이",h_length_x, h_length_y, h_length_z)

        obj = bpy.context.active_object
        if h_length_y > h_length_x * recommand_scale_y * 1.1:
            print("!!!!!!!!!!!!y")
            new_scale = h_length_y = recommand_scale_y * 1.1 / h_length_y
            obj.scale[1] *= new_scale
            self.expansion_ratio = (self.expansion_ratio[0], self.expansion_ratio[1] * new_scale, self.expansion_ratio[2])
        elif h_length_y < h_length_x * recommand_scale_y * 0.9:
            print("!!!!!!!!!!!!y")
            new_scale = h_length_y = recommand_scale_y * 0.9 / h_length_y
            obj.scale[1] *= new_scale
            self.expansion_ratio = (
            self.expansion_ratio[0], self.expansion_ratio[1] * new_scale, self.expansion_ratio[2])

        if h_length_z > h_length_x * recommand_scale_z * 1.1:
            print("!!!!!!!!!!!!z1")
            new_scale = h_length_y = recommand_scale_z * 1.1 / h_length_z
            obj.scale[2] *= new_scale
            self.expansion_ratio = (self.expansion_ratio[0], self.expansion_ratio[1], self.expansion_ratio[2] * new_scale)
        elif h_length_z < h_length_x * recommand_scale_z * 0.9:
            print("!!!!!!!!!!!!z2")
            new_scale = h_length_y =  recommand_scale_z * 0.9 / h_length_z
            obj.scale[2] *= new_scale
            self.expansion_ratio = (self.expansion_ratio[0], self.expansion_ratio[1], self.expansion_ratio[2] * new_scale)
